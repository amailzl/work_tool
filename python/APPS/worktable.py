...
class Sub_W():
    def __init__(self,init_window_name,sq_helper,sq_table):
        self.sq_table = sq_table
        self.enable = True
        self.init_window_name = init_window_name
        self.WID = ''
        self.sq_helper = sq_helper
    ...
    def set_init_window(self, is_update = False, WID = ''):
        ...
        if (is_update == False) :
            ...
            ret = self.sq_helper.select(self.sq_table, sq_select_cmd[1], WID)
            self.WID = WID
            for r in ret :
                self.pending_work_text.insert(END, r[0])
                self.comboxlist.current(process_value.index(r[1]))
