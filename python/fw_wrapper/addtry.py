...
def formatjavadoc(srcfile) : 
    ...
    while contents_index < len(contents) :
        ...
        if "get" in line and "public" in line and "{" in line and block_count == 1 and "static" not in line :
            return_type = line.split("get")[0].split("public")[1].strip().replace("int", "Integer").replace("float", "Float").replace("Result<", "").replace(">", "")
        ...
        if "VehicleController.getInstance().get" in line and get_start:
            newline = "            try {\n\
    "+ line +"\
            } catch (UnavailableSignalValueException e) {\n\
                return new Result<"+return_type+">(("+return_type+") e.getUnavailableSignalValue(), Result.STATUS_UNAVAILABLE);\n\
            }\n"
            line_num = contents.index(line)
            del contents[line_num]
            contents.insert(line_num, newline)
