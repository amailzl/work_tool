...
int usage() {
    printf("Usage:\n\tcs_canframereplayer -d /dev/ICU_CMD -f canmsg.asc -r 100 -t 2"
           "\n\t -d: device name"
           ...
           "\n\t -r: repeat times"
           "\n\t -t: max time gap between cmds writing(ms),1s(1000ms) by default, valid between[2, 1000] "
           "\n");
    return -1;
}
...
int main(int argc, char* argv[]){
    ...
    const char *ASC = NULL;
    int REPEAT = 1;
    int MAXGAP = 1*1000*1000;
    while((ch = getopt(argc, argv, "d:f:r:t:")) != -1)
    {
        switch(ch)
        {
            case 'd':
            ...
            case 't':
                MAXGAP = strtoul(optarg, NULL, 0)*1000;
            break;
            default:
                usage();
                return -1;
                ...
                if (line_len > MIN_CAN_LEN) {
                    ...
                    if ((ds != NULL) && (strstr(line, "CANFD") == NULL)) {
                        ...
                        if (spliteString(line, " ", pre_arr, 3)) {
                            ...
                            if (us > MAXGAP)
                                us = MAXGAP; //连续的信号时间差>MAXGAP，则当MAXGAP处理
                            if (us > 2000) {
                                usleep(us-2000);//信号之间时间差>2ms则sleep us-2ms.
